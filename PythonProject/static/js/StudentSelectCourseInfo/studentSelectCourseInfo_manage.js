				...
				if ($("#studentSelectCourseInfoEditForm").form("validate")) {
					...
					if(!$("#studentSelectCourseInfoEditForm").form("validate")) {
						...
						$("#studentSelectCourseInfoEditForm").form({
						    url:"/StudentSelectCourseInfo/update/" + $("#studentSelectCourseInfo_selectId_edit").val(),
						    onSubmit: function(){
								if($("#studentSelectCourseInfoEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	studentSelectCourseInfo_manage_tool = {
		...
		exportExcel: function() {
			$("#studentSelectCourseInfoQueryForm").form({
			    url:"/StudentSelectCourseInfo/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/StudentSelectCourseInfo/update/" + rows[0].selectId,
					type : "get",
					data : {
					...
					success : function (studentSelectCourseInfo, response, status) {
						...
						if (studentSelectCourseInfo) { 
							...
							$("#studentSelectCourseInfo_selectTime_edit").datetimebox({
