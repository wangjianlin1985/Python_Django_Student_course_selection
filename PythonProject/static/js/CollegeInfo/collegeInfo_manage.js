...
function initCollegeInfoManageTool() {
	collegeInfo_manage_tool = {
		...
		search: function() {
			$("#collegeInfo_manage").datagrid("options").queryParams=queryParams; 
			$("#collegeInfo_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#collegeInfoQueryForm").form({
			    url:"/CollegeInfo/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/CollegeInfo/update/" + rows[0].collegeNumber,
					type : "get",
					data : {
